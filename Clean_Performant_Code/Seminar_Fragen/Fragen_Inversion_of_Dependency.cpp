...
namespace DependencyInversionPrinciple01
{
    ...
    {
    private:
        const Relationships& m_relationships;  // Low - Level
...
namespace DependencyInversionPrinciple02
{
    ...
    public:
        FamilyTree(const IRelationshipBrowser& browser) : m_browser{ browser }{}
...
static void test_conceptual_example_dip()
{
    ...
    Relationships relationships{};   // Low-Level Objekt (Stack)

    relationships.addParentAndChild(parent, child1);   // füllen
    relationships.addParentAndChild(parent, child2);

   // relationships.

    const IRelationshipBrowser& browser = relationships;  // Low-Level // Alias (Referenz // Adresse )

    //browser.   // Ausschnitt // Excerpt
    
    FamilyTree tree{ browser };      // High-Level Objekt (Stack)
