...
static void clientCode(std::shared_ptr<ITarget> target) {
    ...
    std::string request{ target->request() };
    std::cout << request << std::endl << std::endl;
}

static void test_conceptual_example_01() {

    std::shared_ptr<Adaptee> adaptee{ std::make_shared<Adaptee>() };  // Incompatible Object

    std::shared_ptr<Adapter> adapter{ std::make_shared<Adapter>(adaptee) }; // Adapter
...
void test_conceptual_example_adapter() {
